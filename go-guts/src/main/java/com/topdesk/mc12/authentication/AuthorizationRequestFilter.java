...
public class AuthorizationRequestFilter implements ContainerRequestFilter {
	...
	public ContainerRequest filter(ContainerRequest request) {
		if (request.getAbsolutePath() != null) { // TODO Temp escape to prevent
													// filtering right now.
			request.setSecurityContext(PlayerContext.create(entities.get()
					.find(Player.class, 1L), uriInfo));
			return request;
		}
		...
		PlayerContext securityContext = sessions.startNew(authenticatePlayer(),
				uriInfo);
	...
	@Transactional
	private <E> List<E> selectByField(String fieldName, String value,
			Class<E> entityClass) {
